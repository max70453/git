<template>
  <v-container fluid>
    <v-row>
      <!-- фильтры -->
      <v-col cols="3" class="filters">
        <Filters :hotels="hotels" @resultData="show"></Filters>
      </v-col>
      <!-- вывод результатов -->
      <v-col v-if="filterData">
        <v-card  v-for="(hotel, index) in filterData" :key="index" elevation="2" class="ma-2 pa-2">
          <h3>{{hotel.name}}</h3>
            <v-row>
              <v-col>
                <v-rating 
              ...
              <v-col cols="4" class="mt-1">
                ...
                <small>{{hotel.reviews_amount}}</small>
                <v-icon>mdi-map-marker</v-icon>
                <small>{{hotel.country}}</small>
              </v-col>
              <v-col class="offset-4">
                <strong>{{hotel.min_price}} - {{hotel.currency}}</strong><br/>
                <small class="purple lighten-5">Цена за 1 ночь</small>
              </v-col>
            </v-row>
            <p>{{hotel.description}}</p>
          </v-card>
      </v-col>
      <v-col v-if="filterData.length == 0">
        <not-found />
      </v-col>
    </v-row>
  </v-container>
